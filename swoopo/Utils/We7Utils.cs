...
namespace Swoopo.Utils
{
	...
	/// 工具类，来自DiscuzNT
	/// </summary>
    public class We7Utils
	{
        public const string ASSEMBLY_VERSION = "2.6.0";

        private static Regex RegexBr = new Regex(@"(\r\n)", RegexOptions.IgnoreCase);
        public static Regex RegexFont = new Regex(@"<font color=" + "\".*?\"" + @">([\s\S]+?)</font>", We7Utils.GetRegexCompiledOptions());
		...
		public static bool IsCompriseStr(string str, string stringarray, string strsplit)
		{
			...
			str = str.ToLower();
			string[] stringArray = We7Utils.SplitString(stringarray.ToLower(), strsplit);
		...
		public static void ResponseFile(string filepath, string  filename, string filetype)
		{
			...
			try
			{
				...
				HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment;filename=" + We7Utils.UrlEncode(filename.Trim()).Replace("+", " "));
		...
		{
            if (!We7Utils.StrIsNullOrEmpty(strContent))
            {
		...
		{ 
			...
			*/
			...
			return We7Utils.SplitString(filelist.ToString(), "\r\n");
		...
		{
			return We7Utils.MakeSureDirectoryPathExists(name);
		}
		...
		public static bool InIPArray(string ip, string[] iparray)
		{
			...
			string[] userip = We7Utils.SplitString(ip, @".");
			for (int ipIndex = 0; ipIndex < iparray.Length; ipIndex++)
			{
				string[] tmpip = We7Utils.SplitString(iparray[ipIndex],@".");
				int r = 0;
                ...
                try
                {
                    ...
                    foreach (string strs in single)
                    {
                        if (strs != "")
                            ...
                            if (We7Utils.IsTime(splitetime[1].ToString()) == false || We7Utils.IsTime(splitetime[0].ToString()) == false)
                               throw new Exception();
                              break;
...
//#endif
        ...
        {
            if (We7Utils.StrIsNullOrEmpty(target))
            {
                target = source;
            }
            else
