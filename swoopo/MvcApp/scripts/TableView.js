...
*/
	...
	this._toggleSelect = function(){
		var c = jQuery(self.container).find('th.marker input[type=checkbox]')[0];
		if(c.checked){
			self.selectAll();
		}else{
			self.unselectAll();
	...
	this._render_framework = function(){
		...
		// debug
		self._debug = jQuery('#' + debug_div_id);
	...
	this._update_meta = function(){
		...
		var marked_count = 0;
		marked_count = jQuery(self.container).find('table.datagrid td.marker input[value!=""]:checked').length;
		jQuery(self.container).find('div.datagrid_meta span.marked_count').html(marked_count);
		jQuery(self.container).find('div.datagrid_meta span.row_count').html(self.rowCount());
	}
	...
	this._after_render = function(){
		var trs = jQuery(self.container).find('table.datagrid>tbody>tr.tv_row');
		trs.each(function(i, tr){
			...
			jQuery(tr).removeClass('odd even marked');
			jQuery(tr).addClass(clz);
		...
		if(!self.display.title){
			jQuery(self.container).find('div.datagrid_meta span.title').hide();
		}else{
			jQuery(self.container).find('div.datagrid_meta span.title').show();
		}
		if(!self.display.count){
			jQuery(self.container).find('div.datagrid_meta span.count').hide();
		}else{
			jQuery(self.container).find('div.datagrid_meta span.count').show();
		}
		if(!self.display.filter){
			jQuery(self.container).find('div.datagrid_meta span.filter').hide();
		}else{
			jQuery(self.container).find('div.datagrid_meta span.filter').show();
		}
		if(!self.display.pager){
			jQuery('#' + self.pager.id).hide();
		}else{
			jQuery('#' + self.pager.id).show();
		}
		if(!self.display.marker){
			jQuery(self.container).find('td.marker, th.marker').hide();
		}else{
			jQuery(self.container).find('td.marker, th.marker').show();
		}
	};
	...
	this.render = function(){
		...
		jQuery(self.container).find('div.datagrid_meta span.title').html(this.title);
		
		// jQuery.html() is really slow in IE
		//jQuery(self.container).find('div.datagrid_div').html(buf.join(''));
		jQuery(self.container).find('div.datagrid_div').each(function(i, e){
		...
		if(self.display.sort){
			...
			var elements = {};
			jQuery(self.container).find('table.datagrid th[field]').each(function(i, th){
				var k = jQuery(th).attr('field');
				if(k != undefined){
					elements[k] = th;
				}
			});
	...
	this.selectAll = function(){
		jQuery(self.container).find('th.marker input').attr('checked', 'checked');
		jQuery(self.container).find('td.marker input').attr('checked', 'checked');
		self._after_render();
	};
	...
	this.unselectAll = function(){
		jQuery(self.container).find('th.marker input').attr('checked', '');
		jQuery(self.container).find('td.marker input').attr('checked', '');
		self._after_render();
	};
	...
	this.getSelected = function(){
		var items = [];
		jQuery(self.container).find('.datagrid td.marker input[value!=""]:checked').each(function(i, cb){
			if(cb.checked){
